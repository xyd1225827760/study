...
class router
{
     ...
     * @return void
     */
    protected function __construct($appName = 'demo', $appRoot = '')//new类后执行该方法
    {
    ...
    {
        $this->pathFix = DIRECTORY_SEPARATOR;//DIRECTORY_SEPARATOR:目录分隔符,linux上就是’/’ windows上是’\’ 
    }
    ...
    {
        $this->basePath = realpath(dirname(dirname(__FILE__))) . $this->pathFix;//realpath() 函数返回绝对路径。 该函数删除所有符号连接(比如 '/./', '/../' 以及多余的 '/'),返回绝对路径名。 若失败,则返回 false。
    }
    ...
    {
        chdir(dirname($this->controlFile));//chdir() 函数把当前的目录改变为指定的目录。
        include $this->controlFile;
        ...
        call_user_func_array(array(&$module, $methodName), $this->params);//相当于$module->$methodName($this->params);
        return $module;
    }
    ...
    public function error($message, $file, $line, $exit = false)
    {
        ...
        $trace = debug_backtrace();//用于返回执行当前代码的文件、行数、函数名、函数参数（用于代码调试）
        extract($trace[0]);//将数组转成变量（键值对形式）
        extract($trace[1]);
        $log .= ", last called by $file on $line through function $function.";
        error_log($log);//将错误存入日志
        ...
        if($exit)
        {
            if($this->config->debug) die($log);//die() 函数输出一条消息，并退出当前脚本。 
	...
	public function auto_load($className)
	{
        ...
        $classFile = $this->appLibRoot . $className . '.class.php';
    ...
    public function loadConfig($moduleName='', $exitIfNone = true)
    {
        ...
        if(in_array($configFile, $loadedConfigs)) return;//函数在数组中搜索给定的值,就是如果$configFile在$loadedConfigs已存在则返回
        $loadedConfigs[] = $configFile;

        global $config;
        if(!is_object($config)) $config = new config();
        include $configFile;//由于config文件只是一段代码块，所以相当于执行config文件
        $this->config = $config;
        return $config;
    }
