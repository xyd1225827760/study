...
class control
{
    ...
    public function parse($moduleName = '', $methodName = '')
    {
        ...
        $currentPWD = getcwd();//getcwd() 函数返回当前目录。
        chdir(dirname($viewFile));//将当前目录改为dirname($viewFile)

        extract((array)$this->view);//将数组变为变量
        ob_start();//打开输出控制缓冲
        include $viewFile;
        $this->output .= ob_get_contents();//函数将缓冲区的内容复制到一个字符串变量中
