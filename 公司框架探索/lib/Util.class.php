...
class Util
{
	...
	/**
	 * 创建一个新的 WebService 服务.
	 *
	 * @param string $wsdl wsdl地址
	 * @retrun object
	 */
	static   $noneedUser = false ;
	static  $username ;
	static  $pass ;
    public $noErro;

	static public function getSoapClient($wsdl)
	{
		if(self::$noneedUser)
		{
            ...
            $client->userLongin($username,$password,$md5key);
		}
		else 
		{ 
			if (empty($_SESSION['soapclient'][$wsdl])) {
	            $client = new SoapClient($wsdl);
	            $_SESSION['soapclient'][$wsdl] = $client;
				$username=  iconv('gbk','utf-8',$_SESSION['userRelateInfo']['LOGIN_ID']);
				$password =   $_SESSION['userRelateInfo']['PASSWORD'];
				$today=date("Y-m-d");
				$md5key=md5($password.$today);
	            $client->userLongin($username,$password,$md5key);
	        }
	        else {//var_dump($_SESSION['soapclient'][$wsdl]);
	            $client = $_SESSION['soapclient'][$wsdl];
                //var_dump($client->SoapClient($wsdl));exit;
                register_shutdown_function(array("Util","isFatalErro"));
               // $this->noErro = false;
                $client->SoapClient($wsdl);
               // $this->noErro = true ;
	            //if (!$res) {throw new Exception('timeout');}
	        }
		}
		...
		if(!isset($soaps[$wsdl]))
        {
            $soaps[$wsdl] = new SoapClient($wsdl);
        }else{
			$soaps[$wsdl]->SoapClient($wsdl);
		}
		return $soaps[$wsdl];
		*/
	}
	 ...
	 * 输出页面提示信息
	 *
	 * @param object $msg 页面提示信息
	 * @param string $type 调用页面类型,参数类型:succeed,error,alert,confirm
	 * @param string $url  跳转页面地址,为空返回前一链接
	 * @param int $time 等待跳转时间
	 */
	static public function page_msg($msg='',$type='confirm',$url='',$time=0){
		$refresh = false;
		switch($type){
			case "succeed":
				$style = "succeed";
	 ...
	 * @param int $time 等待跳转时间
	 */
	static public function alert_msg($msg='',$type=true,$url='/',$lock=true,$time='3000'){
		if($type){
			$type='succeed';
		}else {
			$type='error';
	...
	function convertOwnerId($id)
	{
		if(strlen($id)==18) return $id;
		elseif(strlen($id)!=15) return $id;
		...
		$getNum = 0;
		$getNum += intval($tmp[0])*7;
		$getNum += intval($tmp[1])*9;
		$getNum += intval($tmp[2])*10;
		$getNum += intval($tmp[3])*5;
		$getNum += intval($tmp[4])*8;
		$getNum += intval($tmp[5])*4;
		$getNum += intval($tmp[6])*2;
		$getNum += intval($tmp[7])*1;
		$getNum += intval($tmp[8])*6;
		$getNum += intval($tmp[9])*3;
		$getNum += intval($tmp[10])*7;
		$getNum += intval($tmp[11])*9;
		$getNum += intval($tmp[12])*10;
		$getNum += intval($tmp[13])*5;
		$getNum += intval($tmp[14])*8;
		$getNum += intval($tmp[15])*4;
		$getNum += intval($tmp[16])*2;
	...
	function isTimeOut()
	{
		$OPMAN = $_SESSION['userRelateInfo']['username'];
		if(empty($OPMAN)) 
		{
			#Util::alert_msg("【温馨提示】您的登录已超时，请重新登录！","error", "/index/logout");
			Util::timeOut();
			exit;
		}		
	}
	...
	function timeOut()
	{
		Util::page_msg('[温馨提示]：您的登录已经超时，请重新登录',true,'/index/logout/');
	}
    static function isFatalErro()
    { 
        $erro = error_get_last();
       // print_r($erro);
        if($erro['type']=='1')self::timeOut();
    }
}
?>