...
class DbPdo {
    ...
    */
    ...
    public function connect() {
        if (!isset($this->link) ) {
			...
			if($this->config->persistant) {
				$this->pconnect = true;
				$configs['params'][constant('PDO::ATTR_PERSISTENT')] = true;
			}
			
			try {
				$this->link = new PDO( $this->config->dsn,  $this->config->user, $this->config->password,$configs['params']);
				//$this->link = new PDO("odbc:Driver={SQL Server};Server=129.19.100.191;Database=candor;",'sa','123');
			...
			if (!empty($this->config->encoding)) {
				$this->link->exec("SET NAMES '{$this->config->encoding}'");
			}
	 ...
	 */
	 public function selectLimit($sql, $pageSize, $offset){
		...
		$sqlCount = preg_replace('|select(.*)from|U',"select count(id) as count from",$sqlArr[0]);
		$this->query($sqlCount);
    ...
    */
    ...
    public function getFields($tableName) {
		...
		switch($this->config->driver){
			...
			WHERE
			  TABLE_NAME = :tabName AND TABLE_SCHEMA='".$this->config->name."'";
			  break;
	...
	*/
     ...
     * @return
     */
    private function haveErrorThrowException() {return;
