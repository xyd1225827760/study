<?php
// you can pass this script to PHP CLI to convert your file.

// adapt these 2 paths to your files.
//($src = $argv[1]) || ($src = '../index.js');

($out = $argv[1]) || ($out = '../../js/index.js');

// or uncomment these lines to use the argc and argv passed by CLI :
/*
if ($argc >= 3) {
	$src = $argv[1];
	$out = $argv[2];
} else {
	echo 'you must specify  a source file and a result filename',"\n";
	echo 'example :', "\n", 'php example-file.php myScript-src.js myPackedScript.js',"\n";
	return;
}
*/

require 'class.JavaScriptPacker.php';

$script .= file_get_contents('../jquery-1.3.2.js');
$script .= file_get_contents('../jquery.validator.js');
$script .= file_get_contents('../jquery.timers.js');
$script .= file_get_contents('../jquery.form.js');
$script .= file_get_contents('../jquery.pngFix.js');
$script .= file_get_contents('../application.js');
$script .= file_get_contents('../customize.js');
$script .= file_get_contents('../swfobject.js');
$script .= file_get_contents('../md5.js');


$t1 = microtime(true);

$packer = new JavaScriptPacker($script, 'Normal', true, false);
$packed = $packer->pack();

$t2 = microtime(true);
$time = sprintf('%.4f', ($t2 - $t1) );
echo 'script ', $src, ' packed in ' , $out, ', in ', $time, ' s.', "\n";

file_put_contents($out, $packed);
?>
